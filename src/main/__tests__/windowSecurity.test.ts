...
// a failure as a hole).
//
// The fakes are deliberately structural rather than real Electron objects, which
// is the point of injecting `openExternal` — this suite needs no electron stub.
// They are cast through `never` at the call sites because they implement only the
// members the module actually reads.

const PROD_HTML =
  'C:\\Users\\a b\\AppData\\Local\\Programs\\Template\\resources\\app.asar\\out\\renderer\\index.html'
...
function fakeWindow(id: number) {
  ...
  const mainFrame: FakeFrame = { url: PROD_URL }
  const webContents = {
    id,
    mainFrame,
    isDestroyed: () => false,
    once: (evt: string, cb: (...args: never[]) => unknown) => listeners.set(evt, cb),
    on: (evt: string, cb: (...args: never[]) => unknown) => listeners.set(evt, cb),
    setWindowOpenHandler: (cb: (...args: never[]) => unknown) => listeners.set('windowOpen', cb)
  }
  return {
    webContents,
    listeners,
...
function fakeEvent(win: FakeWindow, opts: { frame?: FakeFrame | null; url?: string } = {}) {
  const senderFrame = opts.frame === null ? null : (opts.frame ?? win.webContents.mainFrame)
  if (opts.url && senderFrame) senderFrame.url = opts.url
  return { sender: win.webContents, senderFrame } as never
}
...
describe('isSenderAllowed', () => {
  ...
  it('rejects a subframe, which inherits the preload', () => {
    ...
    const win = fakeWindow(1)
    registerTrustedWindow(win as never)
    const iframe: FakeFrame = { url: PROD_URL } // same URL, but not === mainFrame
  ...
  it('keys a POSIX install path by host too — the Linux case', () => {
    // The deb and AppImage install under plain POSIX paths with no drive letter
    // anywhere. The "a trusted path starts with C: so no share name can collide"
    // reassurance does not apply at all here.
    const posixHtml = '/opt/Midir/resources/app.asar/out/renderer/index.html'
    const posixUrl = pathToFileURL(posixHtml)

    initWindowSecurity(undefined, posixHtml)
    const win = fakeWindow(1)
    registerTrustedWindow(win as never)
    expect(isSenderAllowed(fakeEvent(win, { url: posixUrl.href }))).toBe(true)
    expect(
      isSenderAllowed(fakeEvent(win, { url: `file://attacker.example${posixUrl.pathname}` }))
    ).toBe(false)
  })
  ...
  it.runIf(process.platform === 'win32')(
    ...
    () => {
      ...
      // property of the PATH, not the platform. A UNC install directory yields a
      // real host and a pathname with no drive letter — so under an origin-based
      // key it was as forgeable as the POSIX installs.
      const uncHtml =
        '\\\\fileserver\\apps\\Template\\resources\\app.asar\\out\\renderer\\index.html'
...
describe('guardIpc', () => {
  function fakeIpcMain() {
    ...
    const listeners = new Map<string, (...args: never[]) => unknown>()
    return {
      handle: (ch: string, fn: (...args: never[]) => unknown) => handlers.set(ch, fn),
      on: (ch: string, fn: (...args: never[]) => unknown) => listeners.set(ch, fn),
      removeListener: (ch: string, fn: (...args: never[]) => unknown) => {
  ...
  it('runs the real handler for a trusted sender', async () => {
    ...
    guardIpc(raw as never).handle('settings:load', inner)
    const result = await raw.handlers.get('settings:load')!(fakeEvent(win), 'arg' as never)
  ...
  it('throws for an untrusted invoke, and never reaches the handler', () => {
    ...
    guardIpc(raw as never).handle('settings:save', inner)
  ...
  it('silently drops an untrusted fire-and-forget send', () => {
    ...
    guardIpc(raw as never).on('close-window', inner)
    raw.listeners.get('close-window')!(fakeEvent(fakeWindow(42)))
    expect(inner).not.toHaveBeenCalled()
  })
  ...
  it('delivers a trusted fire-and-forget send', () => {
    ...
    guardIpc(raw as never).on('minimize-window', inner)
    raw.listeners.get('minimize-window')!(fakeEvent(win))
    expect(inner).toHaveBeenCalled()
  })

  it('remaps removeListener to the wrapper it actually registered', () => {
    const raw = fakeIpcMain()
    const guarded = guardIpc(raw as never)
    const inner = (): void => {}
  ...
  it('passes other ipcMain members through', () => {
    const raw = { ...fakeIpcMain(), somethingElse: () => 'passthrough' }
    expect((guardIpc(raw as never) as unknown as typeof raw).somethingElse()).toBe('passthrough')
  })
})
...
describe('hardenWindow', () => {
  it('denies every child window, opening safe URLs externally instead', () => {
    ...
    hardenWindow(win as never, { allowExternal: true, openExternal })

    const handler = win.listeners.get('windowOpen')!
    expect(handler({ url: 'https://hybrasyl.com/' } as never)).toEqual({ action: 'deny' })
    expect(openExternal).toHaveBeenCalledWith('https://hybrasyl.com/')

    openExternal.mockClear()
    expect(handler({ url: 'file:///C:/Windows/System32/calc.exe' } as never)).toEqual({
      action: 'deny'
    })
    expect(openExternal).not.toHaveBeenCalled()
  })

  it('denies a child window pointed at a string that is not a URL', () => {
    // An scp-style git remote is the house's canonical example: not a parseable
    // URL at all, so it must fall through to deny-without-opening.
    initWindowSecurity(undefined, PROD_HTML)
    const win = fakeWindow(1)
    const openExternal = vi.fn()
    hardenWindow(win as never, { allowExternal: true, openExternal })

    expect(win.listeners.get('windowOpen')!({ url: 'git@github.com:o/r.git' } as never)).toEqual({
      action: 'deny'
    })
    expect(openExternal).not.toHaveBeenCalled()
  })
  ...
  it('never opens externally when the window is not allowed to', () => {
    ...
    const win = fakeWindow(1)
    const openExternal = vi.fn()
    hardenWindow(win as never, { allowExternal: false, openExternal })

    expect(win.listeners.get('windowOpen')!({ url: 'https://hybrasyl.com/' } as never)).toEqual({
      action: 'deny'
    })
    expect(openExternal).not.toHaveBeenCalled()
  })
  ...
  it('blocks navigation away from our content and hands it to the browser', () => {
    ...
    onNavigate(event as never, 'https://example.com/phish' as never)
  ...
  it('allows navigation to our own content, so a dev HMR reload still works', () => {
    ...
    const event = { preventDefault: vi.fn() }
    win.listeners.get('will-navigate')!(event as never, DEV_URL as never)
  ...
  it('blocks a dangerous-scheme navigation without opening it', () => {
    ...
    const event = { preventDefault: vi.fn() }
    win.listeners.get('will-navigate')!(
      event as never,
      'file:///C:/Windows/System32/calc.exe' as never
    )
  ...
  it('blocks navigation to a remote file:// host mirroring our own path', () => {
    ...
    const event = { preventDefault: vi.fn() }
    win.listeners.get('will-navigate')!(event as never, remote as never)
