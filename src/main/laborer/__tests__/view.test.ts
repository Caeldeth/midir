...
describe('the view: tile to screen', () => {
  it('is the middle of the MAP region of the main layout', () => {
    expect(VIEW_CENTRE).toEqual({ x: 311, y: 157 })
  })
  ...
  it('puts Eduardo one tile straight left of the stand tile, lifted to the body', () => {
    ...
    expect(creaturePoint({ x: 2, y: 11 }, { x: 1, y: 12 })).toEqual({ x: 311 - 56, y: 157 - 12 })
  })

  it('puts Antonio up and to the right of the storage stand tile', () => {
    // Stand on (5,8); Antonio on (3,4): two west and four north.
    expect(creaturePoint({ x: 5, y: 8 }, { x: 3, y: 4 })).toEqual({ x: 311 + 56, y: 157 - 81 - 12 })
  })
