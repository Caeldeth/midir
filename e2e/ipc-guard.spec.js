...
// The unit tests in src/main/__tests__/windowSecurity.test.ts prove the POLICY
// against fakes. They cannot prove the WIRING — that guardIpc actually wraps the
// ipcMain the handlers registered on, and that the main window really was
// registered as trusted. A guard that is correct but never installed passes
// every unit test and protects nothing; a guard that is installed but rejects
// our own window passes them too and bricks the app (the module fails closed).
//
// So drive both directions against the real running app. Cribbed from epona's
// e2e/ipc-guard.spec.js, which is where the rogue-window technique came from.
...
test.describe('IPC sender guard', () => {
  ...
  test('an unregistered window is refused, even with our preload at our own URL', async () => {
    ...
    // would succeed. `sandbox: true` matches the main window's preferences.
    const outcome = await electronApp.evaluate(async ({ BrowserWindow }, paths) => {
      ...
      const result = await rogue.webContents.executeJavaScript(
        `window.api.settings.load().then(() => 'ALLOWED').catch((e) => 'REFUSED: ' + e.message)`
      )
      rogue.destroy()
      return result
    }, ROGUE_PATHS)
