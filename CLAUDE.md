...
- **A popup holds the character still, and only a plain notice may be dismissed.** A `0x30` dialog of type 0 or 1 (prose, no choice) is cleared with Escape, then with the Close button at (589, 461) if it stays up; a menu, a text field, and the credential pane (type 9) stop the assistant, because answering one is a choice the player did not make. The rule and the gestures are `dialogScreen.ts`; the walker asks it before a missed step counts as a stall (`checkPopup`), and the Laborer's close step clicks the same button. **An exchange window is not a dialog**: it is `SExchange 0x42` (`model/exchange.ts`), it opens when another player drags an item onto the character, Escape cancels it (never accepts), and the client's one-button alert after the close is local, so the reducer holds an `alert` state from the close for the walker's one Escape at it. The party byte is 0 for the player and 1 for the partner; the document repo's page has it the other way round, and the client binary and Hybrasyl agree on this one.
