...
- **The gold in the bank is in the banker's words.** No packet carries it. The text-entry prompt for Deposit Money ("Lessee... I've got N coins of yours. …") and Withdraw Money ("You have N coins. …") states the balance before the amount is typed; that prompt is the reading (`model/bankGold.ts`, from Eolathe at Antonio, 2026-09-22). The typed amount is the `0x39` answer to the same pursuit (a length byte and the digits), and the balance moves by it only when the next `SStatus` currency block shows the character's gold moved by the same amount; "Failed." moves nothing. It is `bankGold` on the record, apart from `bank`, because a balance is often known when the items are not.
- **A deposit and a withdrawal are a pick and an inventory change, never a bank packet.** Deposit Item is `0x39` pursuit `0x43` → `0x2F` type 5 (`[u16 pursuit 0x53][u8 count][u8 slot]…`, the slots the banker takes) → the pick `0x39` pursuit `0x53` with a `u8` slot → `0x10` remove-inventory of that slot (204 ms). Withdraw is the type-4 list → `0x39` pursuit `0x56` with a `string8` name → `0x0F` add-inventory (237 ms). `model/bankMoves.ts` moves the bank on the record only on the confirming inventory change, inside `BANK_MOVE_WINDOW_MS`, and only on a bank that has been read (WP22).
...
- **A door is the client's table, not a flag.** `SStaticObjectState 0x32` carries `x, y, state, side` and no tile id; the client looks the object's current tile id up in its 66-pair table (`route/doorTable.ts`, read from `Darkages.exe` at `DAT_0068b8b0`) and shows column 1 for state 0, column 0 otherwise, and its collision follows the form shown. A tile in no row is not a door and the packet changes nothing, so Midir changes nothing either. The `side` byte is measured, not named: side 1 is the cell's first static (map-file offset +2), side 0 the second, the opposite of the second protocol source's left/right. The cache stores some doors open, so look up both columns. `model/doors.ts` holds the wire's word per connection and clears on a map change and a loss; `mapGrid.ts` resolves the id when the grid is built (WP31).
- **A right-click on empty ground is a walk; two right presses close together on a creature are an attack.** The client plans a right-click walk itself (breadth-first over the walls it has drawn for the current view) and sends the ordinary `CWalk 0x06` steps, so the walker confirms each tile as it does for a key; it aims at most eight tiles along its own A* path, at the leg's own warp tile and short of every other map's and of every tile something solid stands on, clicks even a single step (a key would turn first), and re-plans from wherever the client stops; the approach beside an NPC clicks the same way, short of every warp (WP35, `clickTurn` in `walker.ts`; the whole leg since 2026-09-22). The client makes its double right-click itself, from a second press under 1000 ms and within 2 px, so `actionLayer.rightClick` waits out `RIGHT_CLICK_GAP_MS` (1500) before any second press: the double is impossible there, not in the callers. What stands where is `model/entities.ts`, from `SDrawObjects 0x07` (retail batches up to 152 objects in one packet; type 0 is a solid monster, 1 walk-through, 2 an NPC with its name), `0x0C`, `0x0E`, and `0x33`, cleared on a new map and on a loss. A tile's ground is at the WP17 view centre (312, 199) plus the isometric offset (`groundPoint` in `laborer/view.ts`); the pane watcher logs every hand right-click with the tile the projection names and the tile the character stops on, which is the check. The mode is the `walkerRightClick` setting, off by default; a stop halts Midir at once and the client finishes the stretch it was given. A destination the graph cannot reach from where the character stands is dimmed in the picker and Go is off, with the reason on hover (`RouteGraph.reachableFrom`, `WalkerDestination.reachable`, WP39): a named map with no warp Midir has seen is a map to walk to once by hand, not a failed walk. With no character on the window nothing is marked, because reachability is from a map.
...
- **A board post does not say which board it is on.** `SBulletin 0x31` type 3 (a post) and 5 (a mail) carry the post id and not the board id; the board is the one the client's newest `CBulletin 0x3B` read asked for, else the index that is open (`boardOfPost` in `model/board.ts`). The index types (2 and 4) share one shape and so do the post types (3 and 5); the type byte says board or mail and travels as its own field. Board 0 is the mailbox, which is whoever is logged in, so the archive keys it `mail:<name>`. A page of the index includes the cursor post (pages overlap by one) and there is no end marker: the oldest post is reached when a page adds no new id. A header never replaces a body in `boards.json`. **A post id is not unique over a board's life**: a post that leaves the board frees its id for the next one (Sabrael, 2026-09-22), so a listed id whose author, date, or subject differ from the post held is a new post, and the old one moves to `${id}~${seenAtMs}` with `displacedAtMs`, never overwritten (`samePost` in `store/boardStore.ts`; the highlight is the one field that moves). The poll's "already read" check counts only the post the board shows under the id (`readPostIds`). The board pane answers posted clicks and no posted key: `W` opened nothing and Down moved no selection (live, 2026-09-22), so the poll is clicks only, at positions the pane watcher measured (`boardPoll.ts`).
- **The doll is Brigid's compositor, asked for by URL.** `icons/doll.ts` is `AislingRenderer` with the drawing taken out: the letters, the three hair passes, the two armour passes (`i`/`j` above 1000), Brigid's `FRONT_ORDER`, the walk sheet's frame 5 mirrored to face down, 111 × 85 with the `w`/`p`/`c`/`g` sheets 27 px further left. `dollService.ts` reads `khanpal.dat` and the five `khan{m|w}*` pairs through dalib-ts, and keeps Brigid's palette rule (≥ 1000 is the same palette, no luminance blend). Gender is the packed body byte's nibble (`shared/doll.ts`); retail sends 255 for no shield. The Equip screen asks `midir-icon://doll/<fields>` and falls back to the placeholder on a 404 (WP37).
- **The legend arrives only when the player opens their profile.** `SSelfLook 0x39` is the one packet that carries the legend, the title, the guild, and the class name, and the client asks for it (`CSelfLook 0x2D`) when its bottom-button pane is built and when the profile is opened, not at every login: 26 recordings hold 40 logins and 6 self-looks. So the profile is opportunistic like the bank, stamped `profileReadAtMs`, and `mergeCharacter` keeps the newer reading across a login that showed none. Before this rule every login wiped the legend, which is why most records had none (2026-09-21).
- **A walk-warp is a step the server answers with the map, not with a move.** The step's `CWalk 0x06` gets its empty `0x32` acknowledgement, then `0x15` and `0x04`, and no `SMove 0x0B`: the map change is that step's reply (339 walk-warps in the recordings). `model/transitions.ts` learns the edge from that — the last confirmed tile plus the oldest unanswered step, since two are often in flight and the server answers in order — and a teleport, a death, or a dialog warp has every step answered and teaches nothing; a spell, item, skill, or dialog answer inside 2 s drops the candidate as well. One plain tile promotes one destination, the one seen most (a gate's bounce is the other), after two crossings; a world-map hop keeps the pane's point per destination. The learned layer is `transitions.json`, laid over the imported graph by `mergeLearned` with `source` on every edge and the wire's `gameName` on every node, and `route/liveGraph.ts` rebuilds the one graph the app holds on every write. A hand edit on the Map tab is a `curation` in the same file, keyed like an edge: accepted enters the graph whatever its count, rejected leaves it whoever put it there, a warp placed by hand is an acceptance plus the rejection of the one it replaces, and the merge reads curations last so a hand edit wins (WP30). The world XML (`route/xmlworld.json`, from `scripts/import-world-xml.mjs` over the world repo's `.ignore/Old*.xml` set and the retail-unchanged production areas) is a provisional layer: every edge a candidate until the wire crosses it once or the user accepts it, because the XML is a tile off where the wire is not (the Town Hall door, y 5 against y 6) (WP24). A map name is ranked in that same merge, weakest first: the XML, the seed list (`route/mapnames.json`, 2387 names, WP38), the wire. The list outranks the XML because the XML names a retail map as Hybrasyl authored it ("Old Mileth Alchemist" for "Mileth Magic Shop"), and it ties with `WorldMap.dat`, which disagrees with it on 260 names: a map the `.dat` names shows the `.dat`'s name and answers to both (`RouteNode.seedName`). The list adds no map of its own, because a name with no edge behind it is a destination the walker cannot route to; and the import leaves out the maps retail does not have — a map id of 30000 or more, and an `Old` or `Undercroft` map the list does not name (`hybrasylOnly`, 105 maps, WP38). The imported files are never written (WP29).
