...
WP1 (scaffold)   COMPLETE
 └── WP2 (protocol: cipher, framing, decoders)   COMPLETE — pure, and the spine
      ...
      │         │         └── WP19 (read-app UI fixes + character redesign)   COMPLETE — complete/19-read-app-polish.md
...
WP29 (learn map transitions from the wire)   COMPLETE — complete/29-learn-transitions.md — the walk-warp rule, a learned layer with provenance
WP30 (map viewer / route inspector)   PART SHIPPED — the viewer and walker debugger; the edit (criterion 5) now has WP29's layer to write to
...
| WP19 | M    | Read-app UI fixes + character redesign    | COMPLETE — `complete/19-read-app-polish.md`  |
...
transitions, map viewer, dynamic doors, and registration-aware routing, the last done: the walker
plans around the Commons its character cannot enter), and WP17 surfaced WP33 and WP34 (both done:
the walker crosses the world map, and clears a dialog, an exchange, and its confirm mid-walk).
...
| WP30 | M    | Map viewer / route inspector    | HTOO-79  | PART SHIPPED 2026-09-22 — `30-map-viewer.md`; the edit is open |
